...
void		per_line(t_configs *data, char *line, int i, int len); // gets called for every line of the map and if an empty line (END OF MAP) returns - also counts the longest line from the map - recognises '\t' and adds 4 (cs 1 '\t' is 4 ' ' (spaces) later - important for converting later in init_map()).
